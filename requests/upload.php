				...
				if (in_array($ext, $valid_formats)) {
					if ($size < (50024 * 50024)) {
						...
						if (move_uploaded_file($tmp, $videoPath . $GetVideoName . '.' . $video_ext)) {
							...
							if ($newVideo) {
								echo '<div class="media-wrapper videoNew UploadedItemNew" id="' . $newVideo['id'] . '"><video id="player-video" width="100%" height="360" style="max-width:100%;" poster="' . $videoTumbnail . '"  controls><source src="' . $base_url . 'uploads/video/' . $GetVideoName . '.mp4" type="video/mp4"></video></div></div>';
							}
						} else {
							echo "Fail upload folder with read access.";
						}
						...
						if (move_uploaded_file($tmp, $sotryPath . $getSotryName)) {
							...
							if ($newdata) {
								...
								if (in_array($exts, $imageExtensions)) {
									echo '<li data-duration="7" data-sid="' . $newdata['s_id'] . '" data-id="' . $newdata['uid_fk'] . '" data-time="' . $newdata['created'] . '"> <img src="' . $base_url . "uploads/stories/" . $newdata['stories_img'] . '"></li>';
								}
								if (in_array($exts, $videoExtensions)) {
									echo '<li> <li class="move_' . $newdata['s_id'] . '"  data-duration="" data-id="' . $newdata['uid_fk'] . '" data-time="' . $newdata['created'] . '"> <video id="aample' . $newdata['s_id'] . '" src="' . $base_url . "uploads/stories/" . $newdata['stories_img'] . '" type="video/webm"></video> </li> </li>';
