			...
			if($proSystemStatus == '1'){
				if($dataUserProType == 1){
						if($Dot->Dot_ProPriceTableAvantagesStatus(2) == '1'){
						   if($Dot->Dot_GetVisitorNotificationNumberStatus($uid) > 0){ 
							   ...
							   $Dot->Dot_UpdateVisitNotificationPlus($uid);
						   }
						   $visitorCount = $Dot->Dot_GetVisitorNotificationNumberStatus($uid);
						}
						if($Dot->Dot_ProPriceTableAvantagesStatus(6) == '1'){
							if($Dot->Dot_GetFavouriteNotificationNumber($uid) > 0){ 
								...
								$Dot->Dot_UpdateFavouritesNotificationPlus($uid);
							}
