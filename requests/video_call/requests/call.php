<?php
require_once('../../../functions/inc.php'); // Plugin config
require_once('../vendor/autoload.php'); // Plugin library  
...
if($dataUsername) {
	...
	if($call['type'] == 0) {
		$cssbg = ' style="background: black;"';
		$cssbc = ' audio-buttons-container';
	} else {
		$cssbg = ' style="background: yellow;"';
		$cssbc = ' audio-buttons-container';
	}
	
	if($call['status'] == 2 || $call['status'] == 3) {
		$cssbg = ' style="background: red"';
	}
...
<head>
<meta charset="UTF-8" />
<title>Call - <?php echo $userInfo['user_fullname']; ?></title>
<link href="<?php echo $base_url; ?>css/style.css" rel="stylesheet" type="text/css">
<script src="<?php echo $base_url;?>requests/video_call/js/jquery.min.js"></script>
...
if(!call_is_ended) { 
	...
	Video.connect(token, {
		...
		video_call_end(1); 
		$('#call-time').html('<span class="call-error"><strong>Error:</strong> '+error.message+'</span>')
	});
	/* Call Timer */
	callTime = 0;
	function countTimer() {
...
function video_call_end(x) {
	...
	$('#remote-container').css('background', 'orange');
