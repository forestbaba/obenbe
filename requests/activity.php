...
if (isset($_POST['f']) && in_array($_POST['f'], $Activitytype)) {
	...
	if ($activity == 'profileInformation') {
		if (isset($_POST['fn']) && isset($_POST['web']) && isset($_POST['ub']) && isset($_POST['lw']) && isset($_POST['unm'])) {
			...
			$newUserName = mysqli_real_escape_string($db, $_POST['unm']);
	...
	if ($activity == 'getNewNotification') {
		...
		if ($GetNotificationAllList) {
			foreach ($GetNotificationAllList as $nAllData) {
				...
				if ($newNotificationReadStatus == '1') {
					$readClass = 'ic_readed';
					$statusRead = '1';
					$readRel = 'readed';
					$readedToolTip = $page_Lang['update_not_readed_notification'][$dataUserPageLanguage];
					$notReadedColorClass = '';}
		...
		if ($GetAllMentionNotifications) {
			foreach ($GetAllMentionNotifications as $mAllData) {
				...
				$notReadedColorClass = 'notReadedColor';
				$readedToolTip = $page_Lang['update_readed_notification'][$dataUserPageLanguage];
				if ($notificationMentionedReadStatus == '0') {
					$readClass = 'ic_readed';
					$statusRead = '1';
					$readRel = 'readed';
					$readedToolTip = $page_Lang['update_not_readed_notification'][$dataUserPageLanguage];
