...
if (isset($_POST) && count($_POST) > 0 ) {
    ...
    if ($validation === true) { 
	    if($insertData['p_t'] == 'donate'){ 
		    $paymentInsert = mysqli_query($db, "INSERT INTO dot_payments(payment_user_id , payment_post_id, order_key, customer_user_id, payment_type,payment_option,amount)VALUES('$uid','".$insertData['post_id']."','".$insertData['order_id']."','".$insertData['customer_user_id']."', '".$insertData['p_t']."', '".$paymentOption."','".$insertData['oam']."')") or die(mysqli_error($db));
		}else if($insertData['p_t'] == 'pm'){
		    $paymentInsert = mysqli_query($db, "INSERT INTO dot_payments(payment_user_id ,  order_key, payment_type,payment_option,pro_type,amount)VALUES('$uid','".$insertData['order_id']."', '".$insertData['p_t']."', '".$paymentOption."','".$insertData['customer_user_id']."','".$insertData['oam']."')") or die(mysqli_error($db));
		}else if($insertData['p_t'] == 'pb'){
		    $paymentInsert = mysqli_query($db, "INSERT INTO dot_payments(payment_user_id ,payment_post_id,  order_key, payment_type,payment_option,product_owner_id,crtid,amount)VALUES('$uid','".$insertData['post_id']."','".$insertData['order_id']."', '".$insertData['p_t']."', '".$paymentOption."','".$insertData['customer_user_id']."','".$insertData['u_c']."','".$insertData['oam']."')") or die(mysqli_error($db));
		}else if($insertData['p_t'] == 'liveDonate'){
		    $paymentInsert = mysqli_query($db, "INSERT INTO dot_payments(payment_user_id ,live_video_id,  order_key, payment_type,payment_option,product_owner_id,amount)VALUES('$uid','".$insertData['post_id']."','".$insertData['order_id']."', '".$insertData['p_t']."', '".$paymentOption."','".$insertData['customer_user_id']."','".$insertData['oam']."')") or die(mysqli_error($db));
		}
