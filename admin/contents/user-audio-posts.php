...
$countSql = mysqli_query($db,"SELECT COUNT(P.user_post_id), P.user_post_id FROM dot_user_posts P INNER JOIN dot_users U
 ...
 WHERE U.user_status = '1' AND post_page_type = 'wall' AND post_type IN('p_audio') GROUP BY P.user_post_id ") or die(mysqli_error($db));
...
$sql = "SELECT DISTINCT P.user_post_id, P.user_id_fk,P.post_type,P.post_created_time,P.hashtag_normal,P.hashtag_diez,P.post_title_text,P.post_text_details,
P.who_can_see_post,P.post_image_id,P.post_link_url,P.post_link_description,P.post_link_title,P.post_link_img,P.post_link_mini_url,P.post_video_id,P.post_audio_id, 
U.user_name, U.user_fullname,U.user_status,U.user_id FROM dot_user_posts P, dot_users U  WHERE U.user_status='1' AND P.user_id_fk=U.user_id AND P.post_type = 'p_audio' ORDER BY P.user_post_id ASC LIMIT $start_from, $limit";  
