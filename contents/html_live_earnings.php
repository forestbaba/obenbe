...
$resulta = mysqli_query($db,"SELECT DAY(FROM_UNIXTIME(payment_time)) - 1 , SUM(user_earning) AS daily_total, COUNT(*) AS ssm FROM `dot_payments` WHERE MONTH(FROM_UNIXTIME(payment_time)) = MONTH(CURDATE()) AND YEAR(FROM_UNIXTIME(payment_time)) = YEAR(CURDATE()) AND payment_status = '1' AND product_owner_id = '$uid' AND payment_type = 'liveDonate' GROUP BY DAY(FROM_UNIXTIME(payment_time)) ORDER BY DAY(FROM_UNIXTIME(payment_time))") or die(mysqli_error($db)); 
  ...
  <div class="earning_monthly_table">
      ...
      <table class="table-general -striped -highlight-row -has-footer">
          ...
          <tbody>
             ...
             <?php 
					 $tdolarEarning =''; 
					 if($earningsDolarLiveBroadCastingThisMonth){
						  $tdolarEarning = '<span class="dl_e">'.$earningsDolarLiveBroadCastingThisMonth.'$</span>';
					 } 
					 $query="SELECT payment_time,payment_post_id,payment_type,customer_user_id,user_earning , month(FROM_UNIXTIME(payment_time)) as month ,year(FROM_UNIXTIME(payment_time)) as year, COUNT(*) AS ssm FROM `dot_payments` WHERE payment_status = '1' AND payment_type = 'liveDonate' AND product_owner_id = '$uid' AND FROM_UNIXTIME(payment_time) > DATE_SUB(NOW(), INTERVAL 1 MONTH) GROUP BY payment_id ORDER BY payment_time DESC";
					 //$query="SELECT payment_time,payment_post_id,payment_type , month(FROM_UNIXTIME(payment_time)) as month ,year(FROM_UNIXTIME(payment_time)) as year, COUNT(*) AS ssm FROM `dot_payments` WHERE payment_status = '1' AND payment_user_id = '$uid' AND FROM_UNIXTIME(payment_time) > DATE_SUB(NOW(), INTERVAL 1 MONTH) GROUP BY DAY(FROM_UNIXTIME(payment_time)) ORDER BY payment_time";
								...
								while($nt=mysqli_fetch_array($qt,MYSQLI_ASSOC)) {
									 ...
									 $totalDolarEarningsnt = $nt['user_earning'];
									 $dolarEarning = '<span class="dl_e">'.$totalDolarEarningsnt.'$</span>';
